...
text=list()
text[[1]]<-readLines("en_US.twitter.txt")
text=lapply(text,FUN=iconv, to='ASCII', sub=' ')
text=lapply(text,FUN= function(x) gsub("'{2}", " ",x))
text=lapply(text,FUN= function(x) gsub("[0-9]", " ",x))

# Read, chunk, parse data, then make corpus, do transformations, make TDM of tri-grams:
myTDM <- makeQuadTDM(text)
rm(twit)
gc()
