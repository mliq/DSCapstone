...
## Average unique word length:
data<-data.frame(names=c("Twitter Corpus", "News Corpus", "Blogs Corpus"), counts=c(tAvg,bAvg,nAvg))
par(mai=c(1,2,1,1))
colors=c("cadetblue3","chocolate2","firebrick1")
barplot(data$counts, col=colors, main="Average Unique Word Length", horiz=TRUE, names.arg=data$names, las=1, axes=FALSE)
axis(1, at=c(0:10),labels=c(0:10))

## Number of Unique Words
data<-data.frame(names=c("Twitter Corpus", "News Corpus", "Blogs Corpus"), counts=c(tuwCount,buwCount,nuwCount))
par(mai=c(1,2,1,1))
barplot(data$counts, col=colors, main="Number of Unique Words", horiz=TRUE, names.arg=data$names, las=1, axes=FALSE)
axis(1, at = c(0,79427,355364,454293), labels = c(0,79427,,355364,454293))

## Line numbers in each Corpus
